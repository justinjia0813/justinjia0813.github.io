...
test("CV page keeps the approved public content and privacy boundary", () => {
  ...
  assert.match(component, /硬科技投资分析师/)
  assert.match(component, /中国一级市场｜人工智能基础设施、先进材料与新能源/)
  assert.match(component, /广东华胥私募基金管理有限公司/)
  assert.match(component, /江苏天汇红优投资管理有限公司/)
  assert.match(component, /投资概览/)
  assert.match(component, /代表性投资与项目/)
  assert.match(component, /代表性研究与观点/)
  assert.match(component, /硅碳负极 · 半固态电池 · 固态电解质/)
  assert.match(component, /推进相关项目投资超过 1 亿元/)
  assert.match(component, /DeepSeek 投资分析/)
  assert.match(component, /AI 芯片散热材料/)
  assert.match(component, /\.\.\/blogs\/AI时代，分析师的能与不能/)
...
test("CV page keeps the reference-inspired single-column and responsive layout", () => {
  ...
  assert.match(styles, /\.jvc-cv-snapshot/)
  assert.match(styles, /\.jvc-cv-investments/)
  assert.match(styles, /\.jvc-cv-writing/)
